...
//        }
    ...
    {
        $input = $request->except('_token','doregister');
        $rule = [
            'user_name' => 'required|email|max:255',
        ];
        $msg = [
            'user_name.required'   => '邮箱必填',
            'user_name.email'   => '邮箱格式错误',
            'user_name.max'    => '长度最多255',
        ];
        $validator = Validator::make($input, $rule, $msg);
        if ($validator->fails()) {
            return redirect('emailregister')->withErrors($validator)->withInput();
        }
        $input['user_pass'] = Crypt::encrypt($input['user_pass']);
        $input['email'] = trim($input['user_name'],'\r\n');
