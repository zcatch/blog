...
class IndexController extends CommonController
{
    ...
    public function index()
    {
        ...
        $userCollect = [];
        if($user){
            // $userCollect = HomeUser::with('collect')->where(['uid'=>$user->user_id])->get(['id'])->toArray();
            // $a = Collect::where('uid',$user->user_id)->get(['id'])->toArray();
            $userCollect = HomeUser::find($user->user_id)->collect()->get(['art_id'])->toArray();
            $userCollect = array_column($userCollect,'art_id');
        }
        $cate_arts = Cate::where('cate_pid','<>',0)->with('article')->get();
        return view('home.index',compact('cate_arts','userCollect','user'));
    }
